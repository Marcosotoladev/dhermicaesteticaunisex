  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        {caracteristicasTecnicas.map((item, index) => (
          <div key={index} className="space-y-2">
            <input
              type="text"
              value={item}
              onChange={(e) => handleArrayChange(index, e.target.value, setCaracteristicasTecnicas, caracteristicasTecnicas)}
              className="w-full p-2 border border-gray-300 rounded"
            />
          </div>
        ))}
        <button
          type="button"
          onClick={() => handleAddArrayItem(setCaracteristicasTecnicas, caracteristicasTecnicas)}
          className="text-blue-500"
        >
      ...
      <div>
        <label>Ventajas de la tecnología</label>
        {ventajasTecnologia.map((item, index) => (
          <div key={index} className="space-y-2">
            <input
              type="text"
              value={item}
              onChange={(e) => handleArrayChange(index, e.target.value, setVentajasTecnologia, ventajasTecnologia)}
              className="w-full p-2 border border-gray-300 rounded"
            />
          </div>
        ))}
        <button
          type="button"
          onClick={() => handleAddArrayItem(setVentajasTecnologia, ventajasTecnologia)}
          className="text-blue-500"
        >
          Añadir ventaja
        </button>
      ...
      {/* Similar para síntomas, resultados, contraindicaciones, cuidadosPre, cuidadosPost */}
